        ...
        if (ret == FLB_EVENT_ENCODER_SUCCESS) {
            ...
            case ES_EVENT_TYPE_NOTIFY_SETOWNER:
                ...
                flb_log_event_encoder_append_body_values(
                    ...
                    FLB_LOG_EVENT_UINT32_VALUE(event.setowner.uid));
                flb_log_event_encoder_append_body_values(
                    encoder,
                    FLB_LOG_EVENT_CSTRING_VALUE("gid"),
                    FLB_LOG_EVENT_UINT32_VALUE(event.setowner.gid));
                flb_log_event_encoder_append_body_cstring(
                    encoder,
                    "target");
                encode_es_file_t(encoder, event.setowner.target);
                flb_log_event_encoder_body_commit_map(encoder);
                break;
            ...
            case ES_EVENT_TYPE_NOTIFY_PROC_CHECK:
                flb_log_event_encoder_body_begin_map(encoder);
                flb_log_event_encoder_append_body_cstring(encoder, "target");
                if (event.proc_check.target) {
                    encode_es_process_t(encoder, event.proc_check.target);
                } else {
                    flb_log_event_encoder_append_body_null(encoder);
                }
                flb_log_event_encoder_append_body_values(
                    encoder,
                    FLB_LOG_EVENT_CSTRING_VALUE("type"),
                    FLB_LOG_EVENT_INT32_VALUE(event.proc_check.type));
                flb_log_event_encoder_append_body_values(
                    encoder,
                    FLB_LOG_EVENT_CSTRING_VALUE("flavor"),
                    FLB_LOG_EVENT_INT32_VALUE(event.proc_check.flavor));
                flb_log_event_encoder_body_commit_map(encoder);
                break;
            case ES_EVENT_TYPE_NOTIFY_PROC_SUSPEND_RESUME:
                flb_log_event_encoder_body_begin_map(encoder);
                flb_log_event_encoder_append_body_cstring(encoder, "target");
                if (event.proc_suspend_resume.target) {
                    encode_es_process_t(encoder, event.proc_suspend_resume.target);
                } else {
                    flb_log_event_encoder_append_body_null(encoder);
                }
                flb_log_event_encoder_append_body_values(
                    encoder,
                    FLB_LOG_EVENT_CSTRING_VALUE("type"),
                    FLB_LOG_EVENT_INT32_VALUE(event.proc_suspend_resume.type));
                flb_log_event_encoder_body_commit_map(encoder);
                break;
            ...
            case ES_EVENT_TYPE_NOTIFY_PROFILE_REMOVE:
                if (event.profile_remove) {
                    flb_log_event_encoder_body_begin_map(encoder);
                    flb_log_event_encoder_append_body_cstring(encoder, "instigator");
                    if (event.profile_remove->instigator) {
                        encode_es_process_t(encoder, event.profile_remove->instigator);
                    } else {
                        flb_log_event_encoder_append_body_null(encoder);
                    }
                    flb_log_event_encoder_append_body_cstring(encoder, "profile");
                    ...
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("uuid"),
                        FLB_LOG_EVENT_STRING_VALUE(event.profile_remove->profile->uuid.data,
                                                   event.profile_remove->profile->uuid.length));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("install_source"),
                        FLB_LOG_EVENT_INT32_VALUE(event.profile_remove->profile->install_source));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("organization"),
                        FLB_LOG_EVENT_STRING_VALUE(event.profile_remove->profile->organization.data,
                                                   event.profile_remove->profile->organization.length));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("display_name"),
                        FLB_LOG_EVENT_STRING_VALUE(event.profile_remove->profile->display_name.data,
                                                   event.profile_remove->profile->display_name.length));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("scope"),
                        FLB_LOG_EVENT_STRING_VALUE(event.profile_remove->profile->scope.data,
                                                   event.profile_remove->profile->scope.length));
                    flb_log_event_encoder_body_commit_map(encoder);
                    if (msg->version >= 8) {
                        flb_log_event_encoder_append_body_cstring(encoder, "instigator_token");
                        encode_audit_token_t(encoder, &event.profile_remove->instigator_token);
                    }
                    flb_log_event_encoder_body_commit_map(encoder);
                }
                break;
            ...
            case ES_EVENT_TYPE_NOTIFY_TCC_MODIFY:
                if (event.tcc_modify) {
                    ...
                    flb_log_event_encoder_append_body_values(
                        ...
                        FLB_LOG_EVENT_STRING_VALUE(event.tcc_modify->service.data,
                                                   event.tcc_modify->service.length));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("identity"),
                        FLB_LOG_EVENT_STRING_VALUE(event.tcc_modify->identity.data,
                                                   event.tcc_modify->identity.length));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("identity_type"),
                        FLB_LOG_EVENT_INT32_VALUE(event.tcc_modify->identity_type));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("update_type"),
                        FLB_LOG_EVENT_INT32_VALUE(event.tcc_modify->update_type));
                    flb_log_event_encoder_append_body_cstring(encoder, "instigator_token");
                    encode_audit_token_t(encoder, &event.tcc_modify->instigator_token);
                    flb_log_event_encoder_append_body_cstring(encoder, "instigator");
                    if (event.tcc_modify->instigator) {
                        encode_es_process_t(encoder, event.tcc_modify->instigator);
                    } else {
                        flb_log_event_encoder_append_body_null(encoder);
                    }
                    flb_log_event_encoder_append_body_cstring(encoder, "responsible_token");
                    if (event.tcc_modify->responsible_token) {
                        encode_audit_token_t(encoder, event.tcc_modify->responsible_token);
                    } else {
                        flb_log_event_encoder_append_body_null(encoder);
                    }
                    flb_log_event_encoder_append_body_cstring(encoder, "responsible");
                    if (event.tcc_modify->responsible) {
                        encode_es_process_t(encoder, event.tcc_modify->responsible);
                    } else {
                        flb_log_event_encoder_append_body_null(encoder);
                    }
                    flb_log_event_encoder_append_body_values(
                        ...
                        FLB_LOG_EVENT_INT32_VALUE(event.tcc_modify->right));
                    flb_log_event_encoder_append_body_values(
                        encoder,
                        FLB_LOG_EVENT_CSTRING_VALUE("reason"),
                        FLB_LOG_EVENT_INT32_VALUE(event.tcc_modify->reason));
                    flb_log_event_encoder_body_commit_map(encoder);
                }
                break;
            default:
